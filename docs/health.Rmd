...
#overdose YPLL
```{r, eval = FALSE}
total_deaths <- read_tsv("data/health/opioid/total_deaths.txt")

total_deaths %<>% clean_wonder()

non_overdose_deaths <- read_tsv("data/health/opioid/non_overdose_deaths.txt")

non_overdose_deaths %<>% clean_wonder()

suppressed_overdoses <- wonder_time_single("data/health/opioid")
...
```{r}
homocide <- read_tsv("data/health/homocide/total.txt")
...
graph_trendline(
  ...
  "homocide_rate",
  xmin = 2000,
  plot_title = "Homocide Rate per 100,000",
  rollmean = 3,
...
```{r}
#Overdose contribution to health index
#--need by single year age
#--download data for deaths from all other sources, then subtract deaths from all sources except overdoses. #Substract.
```