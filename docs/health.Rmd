...
data %<>% bind_df(insurance) %>%select(-baseline, -current, -city) %>% pull_peers_FIPS()
...
rank_and_nb_group(data[data$year == 2017,],
                  'insured_rate',
                  plot_title = 'Health Insurance, 2017')
```
...
#Other data (not for Thursday)

## Overdose Hypothetical (not in report)
```{r}
data %<>%
...
## YPLL with and without overdoses (not in report) {.tabset}
...
## Health Index with and without overdoses (nt in report){.tabset}
