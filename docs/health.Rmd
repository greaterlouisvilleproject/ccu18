...
brfss_results <- full_join(brfss_results, brfss_results_race, by = c("MSA", "year"))
...
#Create index
health_index <- full_join(mortality, natality, by = c("FIPS", "year")) %>% select(-city)
...
health_z <- health_z %>%
  group_by(year) %>%
  mutate(ypll_index = -1 * norm_z(ypll),
         pct_underweight_index = -1 * norm_z(pct_underweight),
         poor_or_fair_index = -1 * norm_z(poor_or_fair),
         physdays_index = -1 * norm_z(physdays),
         mentdays_index = -1 * norm_z(mentdays)) %>%
  ungroup()
