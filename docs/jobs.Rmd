...
## 1. Ranking
```{r data9}
earn_data_2000 <- read_csv("data/jobs/DEC_00_SF3_P085_with_ann.csv", skip = 1)
...
earn_data <- bind_rows(earn_data_2000, earn_data_05_17)

data <- earn_data

data %<>% pull_peers_FIPS() %>% select(FIPS, city, year, current, baseline, everything())

rm(earn_data_2000, earn_data_05_16, earn_data_17, earn_data_05_17, earn_data)
```
...
```{r graph11}
map_tract@data <- full_join(map_tract@data, earn_map, by = c('GEO_ID' = 'Id'))
...
graph_trendline_race_peer_two(
  data, 
  vars = c('median_earnings_white', 'median_earnings_black'),
  plot_title = 'Median Earnings by Race',
  rollmean = 3,
  y_title = 'Dollars',
  xmax = 2017)
```
...
graph_trendline_race_peer_two(
  ...
  plot_title = "Median Earnings by Sex",
  y_title = "Dollars",
  xmin = 2000,
  xmax = 2017)
```
...
# Job Distribution
```{r data14}
setwd("C:/Users/Harrison Kirby/Desktop/GLP/ccu18")
...
```{r graph14}
map_zip@data %<>% left_join(map_data_zip, by = c('ZIPCODE' = 'zip'))
```
...
```{r data15.1}
#White
unemp_white_00 <- read_csv("data/jobs/DEC_00_SF3_P150I_with_ann.csv", skip = 1)
unemp_white_05_17 <- acs_time("/data/jobs/B23002H/", starting_year = 2005)
...
unemp_white_00 %<>%
  mutate(
    ...
    total = 
      `Male: - In labor force: - Civilian:` +
      `Female: - In labor force: - Civilian:`,
    
    unemp_white = 
      (`Male: - In labor force: - Civilian: - Unemployed` +
      `Female: - In labor force: - Civilian: - Unemployed`) 
      / total * 100) %>%
...
unemp_white <- bind_rows(unemp_white_00, unemp_white_05_17)

#Black
unemp_black_00 <- read_csv("data/jobs/DEC_00_SF3_P150B_with_ann.csv", skip = 1)
unemp_black_05_17 <- acs_time("/data/jobs/B23002B/", starting_year = 2005)
...
unemp_black <- bind_rows(unemp_black_00, unemp_black_05_17)
...
rm(unemp_white, unemp_white_00, unemp_white_05_17, unemp_white_00, unemp_black, unemp_black_05_17, unemp_hisp)

```

```{r graph 10.3, fig.width=10}
graph_trendline_race_peer_two(
  data, 
  vars = c('unemp_white', 'unemp_black'),
  plot_title = 'Unemployment by Race',
  rollmean = 3,
  y_title = 'Percent',
  xmax = 2017)
```
...
```{r}
occupation_00 <- read_csv("data/jobs/DEC_00_SF3_P050_with_ann.csv", skip = 1)
occupation_05 <- acs_time("/data/jobs/B24010/05/", starting_year = 2005)
occupation_10 <- acs_time("/data/jobs/B24010/10/", starting_year = 2010)
occupation_15 <- acs_time("/data/jobs/B24010/15/", starting_year = 2015)
...
occupation_00 %<>%
  mutate(
    ...
    high_wage =
      `Male: - Management, professional, and related occupations:` +
      `Male: - Service occupations: - Protective service occupations:` +
      `Male: - Construction, extraction, and maintenance occupations: - Installation, maintenance, and repair occupations` +
      `Female: - Management, professional, and related occupations:` +
      `Female: - Service occupations: - Protective service occupations:` +
      `Female: - Construction, extraction, and maintenance occupations: - Installation, maintenance, and repair occupations`,
    
    high_wage_per = high_wage / total * 100)
...
occupation_05 %<>%
  mutate(
    ...
    high_wage = 
      `Estimate; Male: - Management, professional, and related occupations:` +
      `Estimate; Male: - Service occupations: - Protective service occupations: - Law enforcement workers including supervisors` +
      `Estimate; Male: - Construction, extraction, maintenance, and repair occupations: - Installation, maintenance, and repair occupations:` + 
      `Estimate; Female: - Management, professional, and related occupations:` +
      `Estimate; Female: - Service occupations: - Protective service occupations: - Law enforcement workers including supervisors` +
      `Estimate; Female: - Construction, extraction, maintenance, and repair occupations: - Installation, maintenance, and repair occupations:`,
...
occupation_10 %<>%
   mutate(
    ...
    high_wage = 
      ...
      `Estimate; Male: - Service occupations: - Protective service occupations: - Law enforcement workers` +
      `Estimate; Male: - Service occupations: - Protective service occupations: - First-line supervisors of law enforcement workers` +
      ...
      `Estimate; Female: - Service occupations: - Protective service occupations: - Law enforcement workers` +
      `Estimate; Female: - Service occupations: - Protective service occupations: - First-line supervisors of law enforcement workers` +
...
occupation <- bind_rows(occupation_00, occupation_05, occupation_10, occupation_15) %>% 
  weight_stl("high_wage_per", "total") %>% pull_peers_FIPS(add_info = FALSE)

data %<>% bind_df(occupation)

rm(occupation_00, occupation_05, occupation_10, occupation_15, occupation)
```

```{r, eval = FALSE}
occupation_05 <- acs_time("/data/jobs/B24010B/05/", starting_year = 2005)
occupation_10 <- acs_time("/data/jobs/B24010B/10/", starting_year = 2010)
occupation_15 <- acs_time("/data/jobs/B24010B/15/", starting_year = 2015)
...
occupation <- bind_rows(occupation_00, occupation_05, occupation_10, occupation_15) %>% 
  weight_stl("high_wage_per", "total") %>% 
  rename(high_wage_black = high_wage_per)
  pull_peers_FIPS(add_info = FALSE)
```
...
write_csv(output, "output_data/jobs.csv")
```

```{r data15.2, eval=FALSE}
#custom graph to include 3-year hispanic data
