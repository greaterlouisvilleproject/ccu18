...
burden_df_total <- svyby(~hcost_burdened, 
                   ~FIPS+year, 
                   design = burden_svy, 
                   svymean, 
                   na.rm = TRUE)
...
burden_df_rent <- svyby(~hcost_burdened, 
                   ~FIPS+year+rent, 
                   design = burden_svy, 
                   svymean, 
                   na.rm = TRUE)
...
burden_df_rent %<>%
  select(-se) %>%
  mutate(hcost_burdened = hcost_burdened * 100) %>%
  spread(key = rent, value = hcost_burdened) %>%
  transmute(
    FIPS, year,
    hcost_burdened_homeowner = `0`,
    hcost_burdened_renter = `1`)
  
burden_df_race <- svyby(~hcost_burdened, 
                   ~FIPS+year+race, 
                   design = burden_svy, 
                   svymean, 
                   na.rm = TRUE)
...
burden_df_rent_race <- svyby(~hcost_burdened, 
                   ~FIPS+race+year+rent, 
                   design = burden_svy, 
                   svymean, 
                   na.rm = TRUE) 
...
graph_trendline_race_peer(
  data, 
  vars = c('hcost_burdened_white', 
           'hcost_burdened_black', 
           'hcost_burdened_hisp'),
...
graph_trendline_race_peer_two(
  data,
  vars = c("hcost_burdened_homeowner", "hcost_burdened_renter"),
...
## 16. Child Poverty Ranking

## 17. Child Poverty Trendline
