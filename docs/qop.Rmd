...
map_tract@data$population_change_pos <- replace(map_tract@data$population_change, map_tract@data$population_change < 0 , NA)

map_tract@data$population_change_pos_log <- log(map_tract@data$population_change_pos)
map_tract@data$population_change_log <- map_tract@data$population_change - min(map_tract@data$population_change, na.rm = TRUE) + 1
map_tract@data$population_change_log <- log(map_tract@data$population_change_log)



map_tract@data$population_change_neg <- replace(map_tract@data$population_change, map_tract@data$population_change > 0 , NA)
...
map_tract@data$population_change_log
...
ggplot(housing_pie, aes(x = race, y = percent, fill = variable)) +
  ...
  labs(title = "Homeownership and Affordability", 
       x = "",
       y = "", 
...
child <- child_pov_data %>% pull_peers_FIPS()

data %<>% bind_df(child_pov_data)
```

## 16. Ranking
```{r}
rank_and_nb_group(child[child$year == 2017,],
                  "child_per", 
