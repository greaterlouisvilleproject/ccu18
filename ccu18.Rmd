...
Read in data and construct conuties from PUMAs. PUMAs are identified using State FIPS codes and PUMA numbers. PUMAs boundaries were redrawn in 2012 based on the 2010 census, so 
```{r}
...
```{r}
acs %<>%
  mutate(assoc = if_else(EDUCD > 80, 1, 0),
         assoc = replace(assoc, EDUCD == 1 , NA),
         bach = if_else(EDUCD > 100, 1, 0),
         bach = replace(bach, EDUCD == 1 , NA))

acs$assoc[acs$AGE <= 24 | acs$AGE >= 35] <- NA
acs$bach[acs$AGE <= 24 | acs$AGE >= 35] <- NA
```
...
results <- svyby(~assoc+bach+unemp, ~FIPS+YEAR+race, design = survey_df, svymean, na.rm = TRUE)

output <- results %>%
  select(FIPS, race, year = YEAR, assoc, bach, unemp) %>%
  mutate(assoc = assoc * 100,
         unemp = unemp * 100,
         bach = bach * 100) %>%
  {
    x <- .
...
graph_trendline(output, 
                "bach_b",
                plot_title = "Bachelor's Degrees of Higher, Black, Ages 25-34",
                rollmean = 3,
                xmin = 2005,
                xmax = 2015,
                y_title = "Percent")
